...
namespace ownership
{
        ...
        Coin_t & coinsMax_OutParam)
    {
        ...
        M_ASSERT_OR_LOGANDTHROW_SS((strVec.size() == 2),
        ...
        M_ASSERT_OR_LOGANDTHROW_SS((coinsMax_OutParam >= coinsMin_OutParam),
            "non_player::ownership::ChanceFactory::Make_Coins() looked up \""
            << KEY_STR << "\", retrieving \"" << VALUE_STR
            << "\" which failed to parse the second comma sep field into a valid (>= the min"
            << " of " << coinsMin_OutParam << ") number of coins.  coinsMax="
            << coinsMax_OutParam);
    }
                                              ...
                                              const non_player::CharacterPtr_t CHARACTER_PTR)
    {
        ...
        for (auto const & NEXT_WEAPONINFO_CHANCE_PAIR : WEAPON_SET.chanceMap)
        {
                ...
                NEXT_WEAPONINFO_CHANCE_PAIR.first.is_dagger)
            {
                ...
                {
                    Make_KnifeOrDaggerSizeChances(CHARACTER_PTR->RankClass(),
                                                  weaponChances.knife.size_map);
                }
    ...
    void ChanceFactory::LookupPossibleWeaponsByRole(const creature::role::Enum ROLE,
        WeaponSetVec_t & weaponSetVec_OutParam)
    {
        using namespace boost::algorithm;
            ...
            for (auto const & NEXT_INSTRUCTION_STR : instructionsVec)
            {
                if (boost::algorithm::starts_with(NEXT_INSTRUCTION_STR, "["))
                {
                    ...
                    if (boost::algorithm::starts_with(NEXT_INSTRUCTION_STR, PICK_CSTR))
                    {
                        ...
                        auto const COUNT_STR{ erase_first_copy(erase_first_copy(
                            NEXT_INSTRUCTION_STR, PICK_CSTR), "]") };
                        ...
                        M_ASSERT_OR_LOGANDTHROW_SS((nextWeaponSet.count > 0),
                            "non_player::ownership::LookupPossibleWeaponsByRole(role="
                            << ROLE_STR << ") with KEY=\"" << KEY_STR << "\" and VALUE=\""
                            << VALUE_STR_LOWER << "\" found INSTRUCTION=\"" << NEXT_INSTRUCTION_STR
                            << "\" and parsed pick count string to be \"" << COUNT_STR
                            << "\", but failed to convert it into a valid (> zero) std::size_t.  "
                    ...
                    {
                        M_HP_LOG("non_player::ownership::LookupPossibleWeaponsByRole(role="
                            << ROLE_STR << ") with KEY=\"" << KEY_STR << "\" and VALUE=\""
                            << VALUE_STR_LOWER << "\" found INSTRUCTION=\"" << NEXT_INSTRUCTION_STR
                ...
                else
                {
                    ...
                    M_ASSERT_OR_LOGANDTHROW_SS(((partsVec.size() > 1) &&
                                                (partsVec.at(0).size() > 2) &&
                    ...
                    M_ASSERT_OR_LOGANDTHROW_SS((chance > 0.0f),
                        "non_player::ownership::LookupPossibleWeaponsByRole(role="
                        << ROLE_STR << ") with KEY=\"" << KEY_STR << "\" and VALUE=\""
                        << VALUE_STR_LOWER << "\" found INSTRUCTION=\"" << NEXT_INSTRUCTION_STR
                        << "\", weapon name=\"" << WEAPON_NAME << "\", and chance str=\""
                        << CHANCE_STR << "\", but was unable to convert this into a valid "
    ...
    float ChanceFactory::GetFloatFromGameDataFile(const std::string & KEY,
                                                  const float         MIN,
                                                  const float         MAX)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    M_ASSERT_OR_LOGANDTHROW_SS((exceptionWhatStr.empty()),
        ...
        chance::MaterialChanceMap_t &       materialsMap_OutParam)
    {
        ...
        auto chanceCool{ 1.0f / game::GameDataFile::Instance()->GetCopyFloat(
            "heroespath-material-primary-chance-base-Cool-onein") };

        auto chanceMetal{ 1.0f / game::GameDataFile::Instance()->GetCopyFloat(
            "heroespath-material-primary-chance-base-Metal-onein") };

        auto chancePrecious{ 1.0f / game::GameDataFile::Instance()->GetCopyFloat(
        ...
        const non_player::CharacterPtr_t CHARACTER_PTR)
    {
        ...
        auto const MIN_VAL{ game::GameDataFile::Instance()->GetCopyFloat(KEY_BASE + "chance-min") };
        auto const MAX_VAL{ game::GameDataFile::Instance()->GetCopyFloat(KEY_BASE + "chance-max") };
    ...
    void ChanceFactory::RestrictMaterialsByComplexity(
        ...
        chance::MaterialChanceMap_t &)//individual precious material chances are not changed here
    {
