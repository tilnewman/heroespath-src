...
namespace combat
{
        ...
        const bool               WILL_FORCE_HIT)
    {
        ...
        {
            turnInfo.SetMostDamageCreaturePair(std::make_pair(
                CREATURE_EFFECT.GetDamageTotal(), creatureAttackingPtrC));
        }
        ...
        const bool                CAN_ADD_CONDITIONS)
    {
        ...
        auto const IS_NONPLAYER_CHARACTER
        ...
        const creature::CreaturePVec_t & creaturesCastUponPVec)
    {
        M_ASSERT_OR_LOGANDTHROW_SS((SPELL_CPTR != nullptr),
            "combat::FightClub::Cast() was given a null SPELL_CPTR.");

        M_ASSERT_OR_LOGANDTHROW_SS((creaturesCastUponPVec.empty() == false),
            "combat::FightClub::Cast(spell=" << SPELL_CPTR->Name()
        ...
        const creature::CreaturePVec_t & creaturesListeningPVec)
    {
        M_ASSERT_OR_LOGANDTHROW_SS((SONG_CPTR != nullptr),
            "combat::FightClub::PlaySong() was given a null SONG_CPTR.");

        M_ASSERT_OR_LOGANDTHROW_SS((creaturesListeningPVec.empty() == false),
            "combat::FightClub::PlaySong(song=" << SONG_CPTR->Name()
        ...
        for (auto nextCreatureCastUponPtr : creaturesListeningPVec)
        {
            ...
            else
            {
                ...
                creatureEffectVec.push_back( CreatureEffect(
                    nextCreatureCastUponPtr, hitInfoVec) );
                                      ...
                                      CombatDisplayCPtrC_t     COMBAT_DISPLAY_CPTRC)
    {
        ...
        for (auto const NEXT_DEFEND_CREATURE_PTR : listeningCreaturesPVec)
        {
                    ...
                    creature::Stats::With::PlayerNaturalWins)))
            {
                ...
                const ContentAndNamePos CNP(
                    "'s roar panics ", NamePosition::SourceThenTarget);
                ...
                creatureEffectsVec.push_back( CreatureEffect(
                    NEXT_DEFEND_CREATURE_PTR, HitInfoVec_t(1, HIT_INFO)) );
            }
            else
            {
                const ContentAndNamePos CNP(
        ...
        bool &                   didArmorAbsorb_OutParam)
    {
        ...
        isPowerHit_OutParam =
            (creatureAttackingPtrC->IsPlayerCharacter() &&
             STRENGTH_TEST &&
