    ...
    private:
                                  ...
                                  std::map<EnumType, float> & OutParam_EnumChanceMap) const
        {
            ...
            if (enumColonChanceStrVec.size() != 2)
            {
                ...
                ss << "combat::strategy::ChanceFactory::" << FUNCTION_NAME << "() Found invalid subparts string: \"" << SUB_STR << "\".";
            ...
            catch (const std::exception & EX)
            {
                ...
                exceptionSS << "combat::strategy::ChanceFactory::" << FUNCTION_NAME << "()  Threw exceptionn calling T::FromString(\"" << enumColonChanceStrVec.at(0) << "\")  This is probably just a typo in the gamedatafile.  Exception: [" << EX.what() << "].";
            ...
            {
                std::ostringstream ss;
                ss << "combat::strategy::ChanceFactory::" << FUNCTION_NAME << "()  Failed to parse \"" << enumColonChanceStrVec.at(1) << "\" into a valid 'enum:float'.  Resulting float (chance) was: " << CHANCE << ".";
                throw std::runtime_error(ss.str());
            }
