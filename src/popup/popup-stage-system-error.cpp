...
namespace popup
{
    ...
    void PopupStageSystemError::Setup()
    {
        ...
        sfml_util::LoadTexture(
            bgTexture_,
            game::GameDataFile::Instance()->GetMediaPath("media-images-misc-error"));

        sfml_util::Mask(bgTexture_, sf::Color::White);
        bgSprite_.setTexture(bgTexture_, true);
        bgSprite_.setColor( sf::Color(255, 255, 255, 32) );

        auto const HORIZ_RESCALE{
            textRegion_.width / static_cast<float>(bgTexture_.getSize().x) };
