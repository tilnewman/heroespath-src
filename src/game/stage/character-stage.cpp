...
namespace stage
{
    ...
    const sfml_util::gui::ColorSet CharacterStage::GUI_DEFAULT_COLORSET_{
        sf::Color(220, 220, 220),
        sf::Color(220, 220, 220),
        sf::Color(180, 180, 180),
        sf::Color(180, 180, 180) };

    const sf::Color CharacterStage::LIGHT_TEXT_COLOR_{
    ...
    {
        M_ASSERT_OR_LOGANDTHROW_SS((RADIOBUTTON_WRAPPER.PTR_ != nullptr),
            "game::CharacterStage::HandleCallback() given a null RADIOBUTTON_PACKAGE.PTR_.");
    ...
    void CharacterStage::UpdateTime(const float ELAPSED_TIME_SECONDS)
    {
        ...
        if (AreAnyStatsIgnored() && (false == AreAnyAnimNumStillMoving()))
        {
            auto const NEW_COLOR_VAL{
                static_cast<sf::Uint8>(sbInsTextSlider_.Update(ELAPSED_TIME_SECONDS)) };
        ...
        {
            auto const NEW_COLOR_VAL{
                static_cast<sf::Uint8>(nInsTextSlider_.Update(ELAPSED_TIME_SECONDS)) };
    ...
    bool CharacterStage::KeyRelease(const sf::Event::KeyEvent & KEY_EVENT)
    {
        ...
        if (nameTextEntryBoxUPtr_->HasFocus())
        {
            if ((KEY_EVENT.code == sf::Keyboard::Return) ||
                (KEY_EVENT.code == sf::Keyboard::Tab))
            {
    ...
    void CharacterStage::Setup_NameLabel()
    {
        ...
        nInsTextRegionUPtr_->SetEntityPos(
            (SCREEN_WIDTH_ * 0.5f) - (nInsTextRegionUPtr_->GetEntityRegion().width * 0.5f)
            + 45.0f,
    ...
    void CharacterStage::Setup_SpacebarInstructionText()
    {
        ...
        sbInsTextRegionUPtr_->SetEntityPos(
            ...
            sexRadioButtonUPtr_->GetEntityRegion().top +
                sexRadioButtonUPtr_->GetEntityRegion().height + sfml_util::MapByRes(30.0f, 90.0f));
    ...
    void CharacterStage::Setup_FixedStats(
        const sfml_util::gui::TextInfo & STAT_TEXT_INFO)
    {
        ...
        sfml_util::gui::TextRegionUVec_t & textRegionUVec)
    {
        ...
        ss << stats::Traits::Name(TRAIT_ENUM)
            << " - "
            << GameDataFile::Instance()->GetCopyStr(DESC_KEY)
            << "\n\n";
        ...
        sfml_util::gui::TextInfo & textInfo) const
    {
        ...
        std::ostringstream sss;
        sss << "Your current " << stats::Traits::ToString(WHICH_STAT)
            << " is only " << STAT_VALUE << ".";
        ...
        else if (WHICH_ROLE == creature::role::Beastmaster)
        {
            if (WHICH_STAT == stats::Traits::Intelligence)
            {
                auto const REASON_STR{ std::string("A Beastmaster's primary attribute is ") +
        ...
        else if ((WHICH_ROLE == creature::role::Firebrand) || (WHICH_ROLE == creature::role::Sylavin))
        {
            if (WHICH_STAT == stats::Traits::Strength)
            {
                auto const REASON_STR{ std::string("Dragons may be magical creatures, but make") +
        ...
        else if (WHICH_ROLE == creature::role::Thief)
        {
            if (WHICH_STAT == stats::Traits::Speed)
            {
                auto const REASON_STR{ std::string("Thieves are typically physically weaker ") +
                    "than the other adventurers.  Speed will be important for dodging attacks" +
        ...
        else if (WHICH_ROLE == creature::role::Wolfen)
        {
            if (WHICH_STAT == stats::Traits::Strength)
            {
                auto const REASON_STR{ std::string("Wolfens are not in your party of ") +
            ...
            if (WHICH_STAT == stats::Traits::Accuracy)
            {
                auto const REASON_STR{ std::string("A Wolfen's strong jaws are useless if ") +
            ...
            if (WHICH_STAT == stats::Traits::Speed)
            {
                auto const REASON_STR{ std::string("Wolfen's are fast for a reason, they need ") +
    ...
    bool CharacterStage::AreAnyAnimNumStillMoving() const
    {
        ...
        {
            if ((ANIM_NUM_SPTR->IgnoreMe() == false) &&
                (ANIM_NUM_SPTR->IsDoneMoving() == false))
            {
                return true;
            }
        }
        ...
        {
            if ((FIXED_NUM_SPTR->IgnoreMe() == false) &&
                (FIXED_NUM_SPTR->IsDoneMoving() == false))
            {
                return true;
            }
        }
    ...
    void CharacterStage::ProduceAnimatingDigits(const float ELAPSED_TIME_SECONDS)
    {
        ...
        if (isAnimStats_ && (animStatsTimeCounterSec_ > animStatsDelayPerSec_))
        {
            ...
            auto const SMOKE_ANIM_MID_X{
                smokeAnimUPtr_->GetEntityPos().x +
                    (smokeAnimUPtr_->GetEntityRegion().width * 0.5f) - 10.0f };

            auto const SMOKE_ANIM_MID_Y{
                smokeAnimUPtr_->GetEntityPos().y +
                    (smokeAnimUPtr_->GetEntityRegion().height * 0.5f) - 10.0f };

            auto const ANIM_NUM_TARGET_X{
                statsFirstNumPosLeft_ + ((NEXT_VAL < 10) ? 10.0f : 0.0f) - 10.0f };
