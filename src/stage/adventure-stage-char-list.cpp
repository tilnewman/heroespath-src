...
namespace stage
{
    ...
    void AdventureCharacterList::SetupPositions_Bars(const WhichBar WHICH_BAR)
    {
        ...
        for (std::size_t i(0); i < NUM_RECTS; ++i)
        {
            ...
            auto const COLOR_BAR_RIGHT_WHITE_VAL_F{ COLOR_BASE_F
                                                    + ((COLOR_HIGH_F - COLOR_BASE_F) * BAR_RATIO) };
            ...
            {
                auto const MANA_TEXTREGION_LEFT{ (LEFT + (BAR_WIDTH * 0.5f))
                                                 - (manaTextRegionsUVec_[i]->GetEntityRegion().width
                                                    * 0.5f) };

                auto const MANA_TEXTREGION_TOP{ (TOP + (HEIGHT * 0.5f))
                                                - (manaTextRegionsUVec_[i]->GetEntityRegion().height
                                                   * 0.5f)
                                                + NUMBER_TEXT_TOP_SPACER };
