...
namespace creature
{
                       ...
                       const With                 OPTIONS)
    {
        M_ASSERT_OR_LOGANDTHROW_SS((CREATURE_PTR != nullptr),
            "creature::Stats::Ratio() called with a null CREATURE_PTR.");
    ...
    {
        M_ASSERT_OR_LOGANDTHROW_SS((CREATURE_PTR != nullptr),
            "creature::Stats::Roll() called with a null CREATURE_PTR.");
    ...
    {
        M_ASSERT_OR_LOGANDTHROW_SS((CREATURE_PTR != nullptr),
            "creature::Stats::Test() called with a null CREATURE_PTR.");
    ...
    stats::Trait_t Stats::LuckBonus(const CreaturePtr_t CREATURE_PTR)
    {
        return misc::random::Int( static_cast<int>(static_cast<float>(
            CREATURE_PTR->TraitWorking(stats::Traits::Luck)) /
                game::GameDataFile::Instance()->GetCopyFloat("heroespath-fight-stats-luck-adj-ratio")) );
    }
                                          ...
                                          const race::Enum          RACE_ENUM)
    {
        auto const BASE{ static_cast<stats::Trait_t>(
            static_cast<float>(TRAIT_VALUE) * game::GameDataFile::Instance()->
                GetCopyFloat("heroespath-stats-race-bonus-base-adj-ratio")) };

        auto const MINOR{ static_cast<stats::Trait_t>(
            static_cast<float>(BASE) * game::GameDataFile::Instance()->
