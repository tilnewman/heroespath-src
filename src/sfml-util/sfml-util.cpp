...
namespace sfml_util
{
        ...
        sf::Sprite & s, const float WIDTH, const float HEIGHT, const float RESIZE_RATIO)
    {
        ...
        if (s.getGlobalBounds().height > HEIGHT)
        {
        ...
        const sf::Vector2f & PROJECTED_IMAGE_SIZE)
    {
        ...
        finalPosV.y
            = ((FIRST_POS_V.y < SECOND_POS_V.y) ? (SCREEN_HEIGHT + SPRITE_HEIGHT + 1.0f)
        ...
        auto const AV_VERTICAL_EXTENT{ (
            (SECOND_POS_V.y > FIRST_POS_V.y) ? (SCREEN_HEIGHT - FIRST_POS_V.y) : FIRST_POS_V.y) };
        ...
        if (std::abs(finalPosV.x - FIRST_POS_V.x) > SCREEN_WIDTH)
        {
            ...
            finalPosV.x
                = ((FIRST_POS_V.x < SECOND_POS_V.x) ? (SCREEN_WIDTH + SPRITE_WIDTH + 1.0f)
            ...
            auto const AH_HORIZ_EXTENT{
                ((SECOND_POS_V.x > FIRST_POS_V.x) ? (SCREEN_WIDTH - FIRST_POS_V.x) : FIRST_POS_V.x)
            };
...
*/
    ...
    namespace color
    {
            ...
            const sf::Color & FROM, const sf::Color & TO, const float RATIO_COMPLETE)
        {
            auto const RED{ static_cast<sf::Uint8>(
                ...
                + static_cast<sf::Uint8>(
                      (static_cast<float>(TO.r) - static_cast<float>(FROM.r)) * RATIO_COMPLETE)) };
            auto const GREEN{ static_cast<sf::Uint8>(
                FROM.g
                + static_cast<sf::Uint8>(
                      (static_cast<float>(TO.g) - static_cast<float>(FROM.g)) * RATIO_COMPLETE)) };
            auto const BLUE{ static_cast<sf::Uint8>(
                FROM.b
                + static_cast<sf::Uint8>(
                      (static_cast<float>(TO.b) - static_cast<float>(FROM.b)) * RATIO_COMPLETE)) };
            auto const ALPHA{ static_cast<sf::Uint8>(
                FROM.a
                + static_cast<sf::Uint8>(
                      (static_cast<float>(TO.a) - static_cast<float>(FROM.a)) * RATIO_COMPLETE)) };
        ...
        const std::string ColorToString(const sf::Color & C, const bool WILL_WRAP)
        {
            ...
            std::ostringstream ss;
            ss << ((WILL_WRAP) ? "(" : "") << static_cast<int>(C.r) << ", " << static_cast<int>(C.g)
               << ", " << static_cast<int>(C.b);
