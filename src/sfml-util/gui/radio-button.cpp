...
namespace sfml_util
{
    namespace gui
    {
        ...
        RadioButton::RadioButton(
            ...
            : TwoStateEntity(
                  std::string(NAME).append("_RadioButton"),
                  POS_LEFT,
                  POS_TOP,
                  SPRITE_UP,
                  SPRITE_DOWN,
        ...
        RadioButton::RadioButton(
            ...
            const bool IS_INVALID)
            : TwoStateEntity(
                  std::string(NAME).append("_RadioButton"), POS_LEFT, POS_TOP, MOUSE_TEXT_INFO)
        {
            if (IS_SELECTED)
            {
                SetIsInFirstState(false);
        ...
        RadioButtonSet::RadioButtonSet(
            ...
            , prevSelection_(TEXT_INFO_VEC.size())
            , // set original value to invalid out-of-range
            callbackHandlerPtrSet_()
        {
            Setup(
                POS_LEFT,
        ...
        RadioButtonSet::RadioButtonSet(
            ...
            , // set to an out-of-range value to mean 'not down in any region'
            invalidSelectionVec_(INVALID_SEL_VEC)
            , prevSelection_(MOUSE_TEXT_INFO_VEC.size())
            , // set original value to invalid out-of-range
            callbackHandlerPtrSet_()
        {
            Setup(
            ...
            const float BETWEEN_PAD)
        {
            ...
            M_ASSERT_OR_LOGANDTHROW_SS(
                (NUM_BUTTONS > 0), entityName_ << " given an empty vector of TextInfo objects.");

            M_ASSERT_OR_LOGANDTHROW_SS(
                (INITIAL_SELECTION < NUM_BUTTONS),
                entityName_
                    << " given an initial selection value larger than the number of buttons="
                    << NUM_BUTTONS << ".");
        ...
        void RadioButtonSet::OnClick(const sf::Vector2f &) { TriggerCallback(); }
        ...
        void RadioButtonSet::OnColorChange() { box_.SetEntityColors(entityColorSet_); }
