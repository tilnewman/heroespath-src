...
namespace sfml_util
{
    namespace gui
    {
        namespace box
        {
            ...
            Box::Box(const std::string & NAME, const box::Info & BOX_INFO)
                : GuiEntity(std::string(NAME).append("_Box"), BOX_INFO.region, BOX_INFO.color_set)
                , boxInfo_(BOX_INFO)
                , topLine_(
                      std::string(NAME).append("Box's_Top Line"),
                      Orientation::Horiz,
                      Side::Top,
                      false)
                , rightLine_(
                      std::string(NAME).append("Box's_Right Line"),
                      Orientation::Vert,
                      Side::Right,
                      false)
                , botLine_(
                      std::string(NAME).append("Box's_Bottom Line"),
                      Orientation::Horiz,
                      Side::Bottom,
                      false)
                , leftLine_(
                      std::string(NAME).append("Box's_Left Line"),
                      Orientation::Vert,
                      Side::Left,
                      false)
                , cTopLeftSprite_(
            ...
            void Box::SetupBox(const box::Info & BOX_INFO)
            {
                ...
                if (HORIZ_MID_LEN > 0)
                    topLine_.Setup(
                        LEFT_TO_USE + cTopLeftSprite_.getLocalBounds().width,
                        TOP_TO_USE,
                ...
                const float BOTTOM(
                    TOP_TO_USE + cTopRightSprite_.getLocalBounds().height + VERT_MID_LEN);
                ...
                if (VERT_MID_LEN > 0)
                    leftLine_.Setup(
                        LEFT_TO_USE,
                        TOP_TO_USE + cTopLeftSprite_.getLocalBounds().height,
                        static_cast<std::size_t>(VERT_MID_LEN));
                ...
                if (boxInfo_.WillBox())
                {
                    ...
                    else
                    {
                        ...
                        if (boxInfo_.frame_lines)
                        {
                            ...
                            sfml_util::DrawRectangle(
                                target, states, outerRect, shadedColor, boxInfo_.line_thickness);
                            ...
                            sfml_util::DrawRectangle(
                                target, states, innerRect, shadedColor, boxInfo_.line_thickness);
                        }
                    }
                }
            }
