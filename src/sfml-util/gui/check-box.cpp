...
namespace sfml_util
{
    namespace gui
    {
        ...
        CheckBox::CheckBox(
            ...
            : TwoStateEntity(
                  std::string(NAME).append("_CheckBox"),
                  POS_LEFT,
                  POS_TOP,
                  SPRITE_UP,
                  SPRITE_DOWN,
        ...
        CheckBox::CheckBox(
            ...
            const bool IS_INVALID)
            : TwoStateEntity(
                  std::string(NAME).append("_CheckBox"), POS_LEFT, POS_TOP, MOUSE_TEXT_INFO)
        {
            if (IS_CHECKED)
            {
                SetIsInFirstState(false);
            ...
            , invalidSelectionsVec_(INVALID_SEL_VEC)
        {
            ...
            M_ASSERT_OR_LOGANDTHROW_SS(
                (NUM_CHECKBOXES > 0), entityName_ << " given an empty vector of TextInfo objects.");

            M_ASSERT_OR_LOGANDTHROW_SS(
                (currentSelections_.size() == NUM_CHECKBOXES),
            ...
            , invalidSelectionsVec_(INVALID_SEL_VEC)
        {
            ...
            for (std::size_t i(0); i < NUM_CHECKBOXES; ++i)
            {
                ...
                checkBoxSVec_.push_back(std::make_shared<CheckBox>(
                    ...
                    POS_TOP + OUTER_PAD + (static_cast<float>(i) * BETWEEN_PAD),
                    MouseTextInfo(
                        TEXT_INFO_VEC[i].up, TEXT_INFO_VEC[i].down, TEXT_INFO_VEC[i].over),
                    currentSelections_[i],
                    IsInvalid(i)));
            }
            ...
            , invalidSelectionsVec_(INVALID_SEL_VEC)
        {
            ...
            M_ASSERT_OR_LOGANDTHROW_SS(
                (NUM_CHECKBOXES > 0), entityName_ << " given an empty vector of string labels.");
        ...
        void CheckBoxSet::PostSetupTasks(
        ...
        void CheckBoxSet::OnColorChange() { box_.SetEntityColors(entityColorSet_); }
