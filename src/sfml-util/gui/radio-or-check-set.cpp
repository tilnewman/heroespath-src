...
#include "sfml-util/gui/radio-or-check-entity.hpp"
    ...
    namespace gui
    {
            ...
            , callbackHandlerPtrOpt_(CALLBACK_HANDLER_PTR_OPT)
        {
            ...
            for (const auto & TITLE_VALID_PAIR : TITLE_VALID_PAIR_VEC)
            {
                ...
                entityUVec_.emplace_back(std::make_unique<sfml_util::gui::RadioOrCheckEntity>(
            ...
            const float BETWEEN_ELEMENTS_VERT_SPACER)
        {
            ...
            for (auto & entityUPtr : entityUVec_)
            {
                ...
                const auto TEXT_SIZE_V { sfml_util::Size(
                    entityUPtr->TextEntity()->GetEntityRegion()) };
