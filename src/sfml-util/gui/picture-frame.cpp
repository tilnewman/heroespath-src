...
namespace gui
{
    ...
    const sf::FloatRect PictureFrame::Setup(const sf::FloatRect & RECT, const sf::Color & COLOR)
    {
        ...
        auto const TOP_RIGHT_POS_LEFT{
            ((outerRect_.left + outerRect_.width) - topRightSprite_.getGlobalBounds().width) + 3.0f };

        topRightSprite_.setPosition(TOP_RIGHT_POS_LEFT, outerRect_.top);

        auto const BOT_LEFT_POS_TOP{
            (outerRect_.top + outerRect_.height) - botLeftSprite_.getGlobalBounds().height };

        botLeftSprite_.setPosition(outerRect_.left, BOT_LEFT_POS_TOP);

        auto const BOT_RIGHT_POS_LEFT{
            (outerRect_.left + outerRect_.width) - botRightSprite_.getGlobalBounds().width };

        auto const BOT_RIGHT_POS_TOP{
            (outerRect_.top + outerRect_.height) - botRightSprite_.getGlobalBounds().height };
...

        {
            ...
            auto const TOP_GAP_HORIZ{
                (outerRect_.width - topLeftSprite_.getGlobalBounds().width) -
...

        {
            ...
            auto const BOT_GAP_HORIZ{
                (outerRect_.width - botLeftSprite_.getGlobalBounds().width) -
...

        {
            ...
            auto const LEFT_GAP_HORIZ{
                (outerRect_.height - topLeftSprite_.getGlobalBounds().height) -
            ...
            for (int i(0); i < LEFT_HORIZ_FILL_COUNT; ++i)
            {
                ...
                auto const OFFSET{ static_cast<float>(i) * LEFT_SPRITE.getGlobalBounds().height };
...

        {
            ...
            auto const RIGHT_GAP_HORIZ{
                (outerRect_.height - topRightSprite_.getGlobalBounds().height) -
