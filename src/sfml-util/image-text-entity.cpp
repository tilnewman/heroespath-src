...
namespace sfml_util
{
    ...
    ImageTextEntity::ImageTextEntity(
        ...
        : ImageTextEntity(
              NAME + "_Constructor2_",
              std::make_unique<sfml_util::ImageEntity>(
                  NAME + "'s_ImageTextEntity_FromConstructor2_'s_", MOUSE_IMAGE_INFO),
              std::make_unique<sfml_util::TextEntity>(
        ...
        if (NEW_MOUSE_STATE != GetMouseState())
        {
            ...
            if (imageEntityUPtr_
                && ((MouseStateSync::Image == mouseStateSync_)
                    || (willSyncImageAndTextMouseState_ && (MouseStateSync::Text == mouseStateSync_)
                        && textEntityUPtr_)))
            {
        ...
        const IEntityPtr_t & IENTITY_CHANGED_PTR)
    {
        ...
        if ((MouseStateSync::Image == mouseStateSync_) || (MouseStateSync::Text == mouseStateSync_))
        {
            Entity::SetMouseState(NEW_MOUSE_STATE);
        }
    }
