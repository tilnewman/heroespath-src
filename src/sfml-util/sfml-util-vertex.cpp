...
// ----------------------------------------------------------------------------
//
// sfml-util-vertex.cpp
//
#include "sfml-util-vertex.hpp"
...
namespace sfml_util
{
    ...
    const std::vector<sf::Vector2f> MakeQuadVertexPositions(
        ...
        const Orientation::Enum ORIENTATIONS_TO_DRAW_FROM_END)
    {
        ...
        if (ORIENTATIONS_TO_DRAW_FROM_END == Orientation::Count)
        {
        ...
        else if (ORIENTATIONS_TO_DRAW_FROM_END == Orientation::Both)
        {
        ...
        else if (ORIENTATIONS_TO_DRAW_FROM_END == Orientation::Horiz)
        {
        ...
        const Orientation::Enum ORIENTATIONS_TO_DRAW_FROM_END)
    {
        ...
        const auto VERTEX_POS_V_VEC { MakeQuadVertexPositions(
            POS_V, sfml_util::Size(TEXTURE_RECT), SIZE_LIMIT_V) };

        const auto TEXTURE_POS_V_VEC { MakeQuadVertexPositions(
            sfml_util::Position(TEXTURE_RECT),
            sfml_util::Size(TEXTURE_RECT),
            SIZE_LIMIT_V,
    ...
    void AppendVertexesForQuadRepeatedOverLength(
        ...
        const Orientation::Enum ORIENTATIONS_TO_DRAW_FROM_END)
    {
        if (((LENGTH > 0.0f) == false) || (ORIENTATION == Orientation::Count))
        {
            return;
        }
        ...
        {
            float pos { ((ORIENTATION == Orientation::Horiz) ? POS_V.x : POS_V.y) };

            const auto POS_LIMIT { pos + LENGTH };

            const auto TEXTURE_LENGTH_OF_ORIENTATION { (
                (ORIENTATION == Orientation::Horiz) ? TEXTURE_RECT.width : TEXTURE_RECT.height) };
