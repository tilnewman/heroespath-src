...
namespace sfml_util
{
    namespace sliders
    {
        ...
        {
            const auto PROGRESS_RATIO { ProgressRatio() };
            const auto POS_LEFT { fromPosV_.x + ((toPosV_.x - fromPosV_.x) * PROGRESS_RATIO) };
            const auto POS_TOP { fromPosV_.y + ((toPosV_.y - fromPosV_.y) * PROGRESS_RATIO) };
