...
//  repeated code.
//
//  This also means that Unique(), Intersection(), and RemoveAFromB() accept different containers
//  types in the same call.  So you can write code such as:   StringHelp::Intersection(myVec,
//  myList, mySet)
//
//  The STL and Boost's string algorithm lib are supposed to be used in conjunction with these
//  functions. So for example there is no join() defined here, because boost's join() simple and
//  avoids repeating code. However, boost's split() is often followed by the same set of operations
//  or customizations, so SplitByChar() is defined here to avoid repeating calls to remove empties
//  or to trim strings.
//
...
// There is a non-const iter, but it is the same as the const, becuase you can't modify the contents
// of a tokenizer. using TokCharIter = TokChar::iterator;
...
namespace stringhelp
{
    ...
    template <typename T>
    bool NumToStdStr(const T NUM, std::string & outStr, const std::string & DEFAULT);
    ...
    // NOTE: This version splits on any chars found in SEP_CHARS, NOT on the entire string as a
    // whole.
    template <typename T>
    ...
    //  CleanStringList(strToClean, false)                       "first,second,,compound
    //  entry,last,," CleanStringList(strToClean,",",true,true," ","{","}")    "{first} {second}
    //  {compound entry} {last}" CleanStringList(strToClean,",",true,false," ","","|")    "first|
    //  second| compound entry| last   |"
    //
    ...
    // An alternative to std::unique() that will also remove non-adjacent duplicates and is also
    // stable. Returns true if any duplicates were removed.
    template <typename T>
    bool Unique(T &);
    ...
    bool Intersection(const A_t & A, const B_t & B, OUT_t & out, const bool WILL_UNIQUE = true);

    // This version returns the result instead
    template <typename A_t, typename B_t>
    const A_t Intersection(const A_t & A, const B_t & B, const bool WILL_UNIQUE = true);
...
namespace stringhelp
{
    ...
    template <typename A_t, typename B_t>
    const A_t Intersection(const A_t & A, const B_t & B, const bool WILL_UNIQUE)
    {
        A_t temp;
    ...
    template <typename A_t, typename B_t, typename OUT_t>
    bool Intersection(const A_t & A, const B_t & B, OUT_t & out, const bool WILL_UNIQUE)
    {
        bool foundAny(false);
