...
namespace talk
{
    ...
    const std::string ChildTalk::Compose(const player::Party & party)
    {
        ...
        switch(misc::random::Int(15))
        {
            case 0:
            {
                return Random(StrVec_t{ "I'm not supposed to", "Not supposed to", "I shouldn't" }) +
                    " talk " +
                    Random(StrVec_t{ "with", "to" }) +
            ...
            case 2:
            {
                return
                    ...
                    Random(StrVec_t{ "my ", "" }) +
                    Random(Parents()) +
                    Random(StrVec_t{ " on you", "" }) +
                    PeriodOrBang();
            }
            case 3:
            {
                return
                    CapFirstLetter(Random(Parents())) +
                    Random(StrVec_t{ "", "!  Strangers", "!  " + Random(PartyNames())}) +
                    "!";
            }
            case 4:
            ...
            case 9:
            {
                return
                    "Do you have " +
                    Random(StrVec_t{ "a " + Random(Relations()), "any " + Random(Relations()) + "s" }) +
                    "?";
            }
            case 10:
            {
            ...
            case 14:
            {
                ...
                const std::string PROP{
                    Random(StrVec_t{ "Dare you to", DAREME, WANNA, "Let's", "Bet you can't", "I'm gonna " }) };
