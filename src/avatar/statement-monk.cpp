...
namespace talk
{
    ...
    const std::string MonkTalk::Compose()
    {
        ...
        switch(misc::random::Int(18))
        {
            case 0:
            {
                return CapFirstLetter(Random(MonkThings())) +
                    " is " +
                    Random(StrVec_t{ "a", "your" }) +
                    " choice.  " +
            ...
            case 1:
            {
                return "May " +
                    Random(MonkThings()) +
                    Random(StrVec_t{ " be with ", " find " }) +
                    Random(StrVec_t{ "you.", "you all."});
            }
            case 2:
            {
                return "Peace " +
                    Random(StrVec_t{ "be with ", "to " }) +
                    Random(StrVec_t{ "you.", "you all." });
            }
            case 3:
            {
            ...
            {
                return "You " +
                    Random(StrVec_t{ "have found ", "speak with " }) +
                    "a humble " +
            ...
            case 8:
            {
                return "Your lives have led you " +
                    Random(StrVec_t{ "here.  ", "to me.  " }) +
                    Random(StrVec_t{ "Think ", "Meditate " }) +
                    "on " +
            ...
            {
                return "I " +
                    Random(StrVec_t{ "follow the ", "seek the " }) +
            ...
            case 14:
            {
                return "Are you considering " +
                    Random(StrVec_t{ "a life ", "life " }) +
                    Random(StrVec_t{ "among ", "with " }) +
                    Random(StrVec_t{ "us?", "the monks?", "the brothers?" });
            }
            case 15:
            {
                return "If I speak of " +
                    Random(MonkThings()) +
                    ", will you " +
