...
namespace talk
{
    ...
    const std::string TownTalk::Compose(
        const player::Party & party,
        const map::Level::Enum LEVEL)
    {
        ...
        auto const TownName{
            [LEVEL]()
            {
                return map::Level::ToString(LEVEL);
            } };
        ...
        if ((misc::random::Int(6) == 0) && (DoesPartyHaveBeasts(party)))
        {
            return
                "They let you " +
                Random(StrVec_t{ "walk around", "in here", "in " + TownName() }) +
                " with " +
        ...
        {
            return
                Random(StrVec_t{ "They ", "The guard ", "The guards " }) +
                Random(StrVec_t{ "allowed ", "let " }) +
                Random(StrVec_t{ "you in here ", "you in this town " }) +
                "with that beast" +
                Random(StrVec_t{ "?", "?!" });
        }
        ...
        switch(misc::random::Int(7))
        {
            ...
            {
                return
                    Random(StrVec_t{ "You're not from ", "Not from " }) +
            ...
            {
                return
                    Random(StrVec_t{ "New to ", "You all must be new to " }) +
                    TownName() +
                    ", eh?";
            }
            case 2:
            {
                return
                    "Welcome to " +
                    TownName() +
                    Random(StrVec_t{ " " + Random(PartyNames()) + PeriodOrBang(), PeriodOrBang() });
            }
            case 3:
            {
                return
                    Random(StrVec_t{ "Are you ", "You " }) +
                    "with the " +
                    Random(StrVec_t{ "town", TownName() }) +
                    " guard?";
            }
            case 4:
            {
                return
                    CapFirstLetter(Random(WeatherAdjectives())) +
                    " " +
                    Random(TimesOfDay()) +
            ...
            case 5:
            {
                return
                    "This is " +
                    Random(StrVec_t{ "such a ", "a " }) +
                    Random(WeatherAdjectives()) +
                    Random(StrVec_t{ " place.", " town." });
            }
            case 6:
            {
                return
                    "Ever " +
                    Random(StrVec_t{ "been to ", "seen " }) +
                    "such a " +
                    Random(WeatherAdjectives()) +
                    Random(StrVec_t{ " place?", " town?" });
            }
