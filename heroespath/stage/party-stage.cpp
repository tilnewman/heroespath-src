...
namespace stage
{
    ...
    :
        ...
        warningTextSlider_    (150, 255, 4.0f, static_cast<sf::Uint8>(sfml_util::rand::Int(150, 255))),
        ouroborosSPtr_        (),
        bottomSymbol_         (),
    ...
    bool PartyStage::HandleCallback(const heroespath::callback::PopupResponse & PACKAGE)
    {
        ResetMouseOverPopupState();
        willShowMouseOverPopup_ = true;
    ...
    {
        heroespath::LoopManager::Instance()->Goto_CharacterCreation();
        return true;
    }
    ...
    void PartyStage::Setup()
    {
        ...
        heroespath::player::CharacterSSet_t charactersSSet( heroespath::state::GameStateFactory::Instance()->LoadAllCharacters() );
        ...
        const float LIST_BOX_WIDTH(heroespath::creature::NameInfo::Instance()->Length(LONGEST_TEXT_INFO) + sfml_util::MapByRes(50.0f, 300.0f));
    ...
    void PartyStage::UpdateTime(const float ELAPSED_TIME_SECONDS)
    {
        ...
        if ((mouseOverPopupTimerSec_ > MOUSE_OVER_POPUP_DELAY_SEC_) && (mouseOverTextureSPtr_.get() == nullptr))
        {
                ...
                (itemSPtr->character_sptr->ImageFilename().empty() == false))
            {
                ...
                mouseOverBoxWidth_ = MOUSE_OVER_IMAGE_PAD_ + sfml_util::gui::CreatureImageManager::Instance()->DimmensionMax() + heroespath::creature::NameInfo::Instance()->Length() + sfml_util::MapByRes(50.0f, 150.0f);
    ...
    void PartyStage::StartNewGame()
    {
        ...
        for (std::size_t i(0); i < player::Party::MAX_CHARACTER_COUNT_; ++i)
        {
            ...
            M_ASSERT_OR_LOGANDTHROW_SS(state::GameStateFactory::Instance()->DeleteCharacter(partyListBoxSPtr_->At(i)->character_sptr), "PartyStage::HandleCallback_StartButton() while trying to heroespath::state::GameStateFactory::DeleteCharacter(\"" << NEXT_CHAR_NAME << "\") failed.");
        }
