...
namespace strategy
{
    ...
    void ChanceFactory::Initialize()
    {
        ...
        for (int i(0); i < creature::race::Count; ++i)
        {
            ...
            M_ASSERT_OR_LOGANDTHROW_SS((VALUE.empty() == false), "heroespath::combat::strategy::ChanceFactory::Initialize()  (while parsing race=\"" << RACE_STR << "\")  failed to find gamedatafile value with key=\"" << KEY << "\"");
            ...
            for (auto const & NEXT_PART_STR : partsVec)
            {
                ...
                M_ASSERT_OR_LOGANDTHROW_SS((subPartsVec.empty() == false), "heroespath::combat::strategy::ChanceFactory::Initialize()  (while parsing race=\"" << RACE_STR << "\")  Failed to parse \"" << NEXT_PART_STR << "\" into any sub strings sep by comma.");
                ...
                else
                {
                    ...
                    ss << "heroespath::combat::strategy::ChanceFactory::ParsePartsString() Didn't find a known subparts title.  Found \"" << subPartsVec.at(0) << "\" instead.";
        ...
        for (int i(0); i < creature::role::Count; ++i)
        {
            ...
            const std::string VALUE( GameDataFile::Instance()->GetCopyStr(KEY) );
            M_ASSERT_OR_LOGANDTHROW_SS((VALUE.empty() == false), "heroespath::combat::strategy::ChanceFactory::Initialize()  (while parsing role=\"" << ROLE_STR << "\")  failed to find gamedatafile value with key=\"" << KEY << "\"");
            ...
            for (auto const & NEXT_PART_STR : partsVec)
            {
                std::vector<std::string> subPartsVec;
                appbase::stringhelp::SplitByChar(NEXT_PART_STR, subPartsVec, ',', true, true);
                M_ASSERT_OR_LOGANDTHROW_SS((subPartsVec.empty() == false), "heroespath::combat::strategy::ChanceFactory::Initialize()  (while parsing role=\"" << ROLE_STR << "\")  Failed to parse \"" << NEXT_PART_STR << "\" into any sub strings sep by comma.");
                ...
                {
                    std::ostringstream ss;
                    ss << "heroespath::combat::strategy::ChanceFactory::ParsePartsString() (while parsing role=\"" << ROLE_STR << "\") Didn't find a known subparts title.  Found \"" << subPartsVec.at(0) << "\" instead.";
                    throw std::runtime_error(ss.str());
                }
            }
    ...
    {
        M_ASSERT_OR_LOGANDTHROW_SS((raceRoleChancesMap_.empty() == false), "heroespath::combat::strategy::ChanceFactory::Get(" << creature::race::ToString(RACE_ENUM) << ", " << creature::role::ToString(ROLE_ENUM) << ") called before Initialize() was called.");
    ...
    void ChanceFactory::ParseSubPartsOutnumberRetreat(const std::vector<std::string> & SUBPARTS_VEC, OutnumberRetreatChanceMap_t & OutParam_OutnumberRetreatChanceMap) const
    {
        for (auto const & NEXT_SUBSTRING : SUBPARTS_VEC)
        {
            ...
            else
            {
                ...
                M_ASSERT_OR_LOGANDTHROW_SS(((countCountColonChanceStrVec.size() == 2) || (countCountColonChanceStrVec.size() == 3)), "heroespath::combat::strategy::ParseSubPartsOutnumberRetreat() Found invalid substring: \"" << NEXT_SUBSTRING << "\".");
                ...
                if (boost::algorithm::contains(countCountColonChanceStrVec.at(0), "-"))
                {
                    ...
                    M_ASSERT_OR_LOGANDTHROW_SS(((rangeNumberStrVec.size() == 1) || (rangeNumberStrVec.size() == 2)), "heroespath::combat::strategy::ParseSubPartsOutnumberRetreat() Found invalid range string: \"" << countCountColonChanceStrVec.at(0) << "\".  This should either be a whole counting number or two of them separated by a dash.");
                    ...
                    if (rangeNumberStrVec.size() == 2)
                    {
                        ...
                        M_ASSERT_OR_LOGANDTHROW_SS((rangeEnd >= 0), "heroespath::combat::strategy::ParseSubPartsOutnumberRetreat() Found invalid range end string: \"" << rangeNumberStrVec.at(1) << "\".  This should be a whole counting number greater than or equal to zero.");
                    }
                    ...
                    M_ASSERT_OR_LOGANDTHROW_SS((rangeStart >= 0), "heroespath::combat::strategy::ParseSubPartsOutnumberRetreat() Found invalid range start string: \"" << rangeNumberStrVec.at(0) << "\".  This should be a whole counting number greater than or equal to zero.");

                    if (rangeEnd > 0)
                        M_ASSERT_OR_LOGANDTHROW_SS((rangeStart < rangeEnd), "heroespath::combat::strategy::ParseSubPartsOutnumberRetreat() Found invalid range string: \"" << countCountColonChanceStrVec.at(0) << "\".  The start was not less than the end.");
                ...
                else
                {
                    ...
                    M_ASSERT_OR_LOGANDTHROW_SS((countCountColonChanceStrVec.at(0) == "dist"), "heroespath::combat::strategy::ParseSubPartsOutnumberRetreat() Found invalid initial:count:chance string: \"" << countCountColonChanceStrVec.at(0) << "\".");
                    ...
                    M_ASSERT_OR_LOGANDTHROW_SS((initialCount >= 0), "heroespath::combat::strategy::ParseSubPartsOutnumberRetreat() Found invalid initial count string: \"" << countCountColonChanceStrVec.at(1) << "\".");
