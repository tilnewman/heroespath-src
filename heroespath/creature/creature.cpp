...
namespace creature
{
    ...
    const std::string Creature::ItemAdd(const item::ItemSPtr_t & ITEM_SPTR)
    {
        ...
        {
            M_HP_LOG("heroespath::creature::ItemAdd(creature_name=\"" << Name() << "\", item_name=\"" << ITEM_SPTR->Name() << "\") item add failed:  \"" << IS_ALLOWED_STR << "\".");
    ...
    const std::string Creature::ItemEquip(const item::ItemSPtr_t & ITEM_SPTR)
    {
        ...
        if (IS_ALLOWED_STR != ITEM_ACTION_SUCCESS_STR_)
        {
            M_HP_LOG("heroespath::creature::ItemEquip(creature_name=\"" << Name() << "\", item_name=\"" << ITEM_SPTR->Name() << "\") item equip failed:  \"" << IS_ALLOWED_STR << "\".");
            return IS_ALLOWED_STR;
        }
