...
namespace title
{
    ...
    {
        M_ASSERT_OR_LOGANDTHROW_SS(((E >= 0) || (E < title::Count)),                     "heroespath::creature::title::TitleWarehouse::Get(enum=" << E << ")_InvalidValueError.");
        M_ASSERT_OR_LOGANDTHROW_SS((static_cast<std::size_t>(E) < titleUPtrVec_.size()), "heroespath::creature::title::TitleWarehouse::Get(\"" << title::ToString(E) << "\") enum given was " << E << " but that was greater than (or equal to) the vec size of " << titleUPtrVec_.size() << ".");
        M_ASSERT_OR_LOGANDTHROW_SS((titleUPtrVec_.empty() == false),                     "heroespath::creature::title::TitleWarehouse::Get(\"" << title::ToString(E) << "\") called before vec was populated by Setup().");
        const TitlePtr_t TITLE_PTR(titleUPtrVec_[E].get());
        M_ASSERT_OR_LOGANDTHROW_SS((TITLE_PTR != nullptr),                               "heroespath::creature::title::TitleWarehouse::Get(\"" << title::ToString(E) << "\") found a nullptr in the vector.");
        return TITLE_PTR;
    }
    ...
    bool TitleWarehouse::Test()
    {
        ...
        if (false == hasInitialPrompt)
        {
            ...
            LoopManager::Instance()->TestingStrAppend("heroespath::creature::title::TitleWarehouse::Test() Starting Tests...");
        }
        ...
        if (titleIndex < title::Count)
        {
            ...
            M_ASSERT_OR_LOGANDTHROW_SS((TITLE_PTR != nullptr),                          "heroespath::creature::title::TitleWarehouse::Test(\"" << title::ToString(NEXT_ENUM) << "\") resulted in a nullptr being returned.");
            M_ASSERT_OR_LOGANDTHROW_SS((TITLE_PTR->Desc().empty() == false),            "heroespath::creature::title::TitleWarehouse::Test(\"" << title::ToString(NEXT_ENUM) << "\") resulted in an empty Desc().");
            M_ASSERT_OR_LOGANDTHROW_SS((TITLE_PTR->LongDesc().empty() == false),        "heroespath::creature::title::TitleWarehouse::Test(\"" << title::ToString(NEXT_ENUM) << "\") resulted in an empty LongDesc().");
            M_ASSERT_OR_LOGANDTHROW_SS((TITLE_PTR->ImageFilename().empty() == false),   "heroespath::creature::title::TitleWarehouse::Test(\"" << title::ToString(NEXT_ENUM) << "\") resulted in an empty ImageFilename().");
            M_ASSERT_OR_LOGANDTHROW_SS((TITLE_PTR->RolesCopy().empty() == false),       "heroespath::creature::title::TitleWarehouse::Test(\"" << title::ToString(NEXT_ENUM) << "\") resulted in an empty RolesVec().");
            M_ASSERT_OR_LOGANDTHROW_SS((TITLE_PTR->Which() == NEXT_ENUM),               "heroespath::creature::title::TitleWarehouse::Test(\"" << title::ToString(NEXT_ENUM) << "\") resulted in a Title with a different tile::Enum (\"" << title::ToString(TITLE_PTR->Which()) << "\")");
            auto const TEXTURE_SPTR{ sfml_util::gui::TitleImageManager::Instance()->Get(NEXT_ENUM) };
            M_ASSERT_OR_LOGANDTHROW_SS((TEXTURE_SPTR.get() != nullptr),                 "heroespath::creature::title::TitleWarehouse::Test(\"" << title::ToString(NEXT_ENUM) << "\") resulted in an empty image/texture.");
            ++titleIndex;
        ...
        LoopManager::Instance()->TestingStrAppend("heroespath::creature::title::TitleWarehouse::Test()  ALL TESTS PASSED.");
        return true;
    }
