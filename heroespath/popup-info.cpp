...
namespace heroespath
{
    ...
    PopupInfo::PopupInfo(const std::string &                     NAME,
                         ...
                         const heroespath::Popup::Enum           TYPE,
    ...
    {
        M_ASSERT_OR_LOGANDTHROW_SS((sfml_util::PopupButtons::IsValid(BUTTONS)), "heroespath::PopupInfo(type=" << heroespath::Popup::ToString(TYPE) << ", buttons=" << sfml_util::PopupButtons::ToString(BUTTONS) << ", image=" << sfml_util::PopupImage::ToString(IMAGE) << ", textInfo=\"" << TEXT_INFO.text << "\") was given a BUTTONS value of " << BUTTONS << ", which is invalid.");
        M_ASSERT_OR_LOGANDTHROW_SS((false == TEXT_INFO.text.empty()),           "heroespath::PopupInfo(type=" << heroespath::Popup::ToString(TYPE) << ", buttons=" << sfml_util::PopupButtons::ToString(BUTTONS) << ", image=" << sfml_util::PopupImage::ToString(IMAGE) << ", textInfo=\"" << TEXT_INFO.text << "\") was given TEXT_INFO.text that was empty.");
        M_ASSERT_OR_LOGANDTHROW_SS((sfml_util::PopupImage::IsValid(IMAGE)),     "heroespath::PopupInfo(type=" << heroespath::Popup::ToString(TYPE) << ", buttons=" << sfml_util::PopupButtons::ToString(BUTTONS) << ", image=" << sfml_util::PopupImage::ToString(IMAGE) << ", textInfo=\"" << TEXT_INFO.text << "\") was given an IMAGE value of " << IMAGE << ", which is invalid.");

        if ((imageFadeSpeed_ > 0.0f) && (textureSVec_.empty()))
        {
            std::ostringstream ss;
            ss << "heroespath::PopupInfo(type=" << heroespath::Popup::ToString(TYPE) << ", buttons=" << sfml_util::PopupButtons::ToString(BUTTONS) << ", image=" << sfml_util::PopupImage::ToString(IMAGE) << ", textInfo=\"" << TEXT_INFO.text << "\") was given an image fade speed of " << IMAGE_FADE_SPEED << " but there were no images in IMAGES_VEC, which makes no sense.  If there is a fade speed then there must be images to fade-in.";
    ...
    :
        ...
        type_            (heroespath::Popup::ImageSelection),
        soundEffect_     (SOUND_EFFECT),
        boxInfo_         (),
        ratioX_          (1.0f),
        ratioY_          (1.0f),
    ...
    :
        ...
        buttons_         (sfml_util::PopupButtons::SelectCancel),
        image_           (sfml_util::PopupImage::Large),
        type_            (heroespath::Popup::NumberSelection),
