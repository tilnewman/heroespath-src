...
namespace chance
{
    ...
    {
        M_ASSERT_OR_LOGANDTHROW_SS((mat_map_pri.empty() == false), "heroespath::non_player::ownership::chances::ItemChances::RandomMaterialPri() called when mat_map_pri is empty.");
    ...
    void ItemChances::SetCountChanceIncrement(const float CHANCE)
    {
        ...
        M_ASSERT_OR_LOGANDTHROW_SS((i < MAX_ITERATIONS), "heroespath::non_player::ownership::ItemChances::SetCountChanceIncrement(" << CHANCE << ") reached the sentinel of " << MAX_ITERATIONS << " iterations.  Something is very wrong...");
    }
