...
namespace ownership
{
                                   ...
                                   item::Coin_t &  coinsMax_OutParam)
    {
        ...
        M_ASSERT_OR_LOGANDTHROW_SS((strVec.size() == 2), "heroespath::non_player::ownership::ChanceFactory::Make_Coins() looked up \"" << KEY_STR << "\", retrieving \"" << VALUE_STR << "\" which failed to be parsed into 2 comma sep strings.");
        ...
        M_ASSERT_OR_LOGANDTHROW_SS((coinsMin_OutParam >= 0), "heroespath::non_player::ownership::ChanceFactory::Make_Coins() looked up \"" << KEY_STR << "\", retrieving \"" << VALUE_STR << "\" which failed to parse the first comma sep field into a valid number of coins.  coinsMin=" << coinsMin_OutParam);
        ...
        M_ASSERT_OR_LOGANDTHROW_SS((coinsMax_OutParam >= coinsMin_OutParam), "heroespath::non_player::ownership::ChanceFactory::Make_Coins() looked up \"" << KEY_STR << "\", retrieving \"" << VALUE_STR << "\" which failed to parse the second comma sep field into a valid (>= the min of " << coinsMin_OutParam << ") number of coins.  coinsMax=" << coinsMax_OutParam);
    }
                                                  ...
                                                  chance::ArmorChances &              armorChances)
    {
        ...
        for (auto const & NEXT_ARMOR_CHANCE_STR : armorChancesVec)
        {
            ...
            M_ASSERT_OR_LOGANDTHROW_SS((piecesVec.size() >= 2), "heroespath::non_player::ownership::ChanceFactory::LookupPossibleArmorByRole(role=\"" << creature::role::ToString(CREATURE_SPTR->Role().Which()) << "\") found value-str=\"" << VALUE_STR << "\" which failed to be parsed into the required 2  or more comma sep fields.");
        ...
        for (auto const & NEXT_WEAPON_SET_STR : weaponSetVec)
        {
            ...
            for (auto const & NEXT_INSTRUCTION_STR : instructionsVec)
            {
                if (boost::algorithm::starts_with(NEXT_INSTRUCTION_STR, "["))
                {
                    ...
                    {
                        M_HP_LOG("heroespath::non_player::ownership::LookupPossibleWeaponsByRole(role=" << ROLE_STR << ") with KEY=\"" << KEY_STR << "\" and VALUE=\"" << VALUE_STR_LOWER << "\" found INSTRUCTION=\"" << NEXT_INSTRUCTION_STR << "\" which was not recognized, and will be ignored.");
                    }
                }
                else
                {
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    M_ASSERT_OR_LOGANDTHROW_SS((exceptionWhatStr.empty()), "heroespath::non-player::ownership::ChanceFactory::GetFloatFromGameDataFile(key=\"" << KEY << "\"), found value \"" << VALUE_STR << "\" that failed to be parsed as a float, throwing exception: \"" << exceptionWhatStr << "\".");
                    return -1.0f;
                }
            }
        }
                                                    ...
                                                    chance::MaterialChanceMap_t &       materialsMap_OutParam)
    {
        ...
        M_ASSERT_OR_LOGANDTHROW_SS((materialsMap_OutParam.empty() == false), "heroespath::non_player::ownership::ChanceFactory::Make_MaterialChancesPrimary(\"" << CREATURE_SPTR->Name() << "\") final materials map was empty.");
    }
