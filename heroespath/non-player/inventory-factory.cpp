...
#include "heroespath/non-player/chance-factory.hpp"
...
namespace ownership
{
    ...
    void InventoryFactory::PopulateCreatureInventory(non_player::CharacterSPtr_t & creatureSPtr)
    {
        ...
        for (auto const & NEXT_ITEM_SPTR : ITEM_SVEC_PAIR.first)
        {
            ...
            {
                M_HP_LOG("heroespath::non-player::ownership::InventoryFactory::PopulateCreatureInventory[to equip - add step](creature=\"" << creatureSPtr->ToString() << "\") unable to add the item \"" << NEXT_ITEM_SPTR->Name() << "\" \"" << NEXT_ITEM_SPTR->Desc() << "\" with reported error \"" << ITEM_ADD_RESULT << "\".  Proceeding...");
            }
            else
            {
                if (NEXT_ITEM_SPTR->Category() & item::category::Equippable)
                {
                    ...
                    if (ITEM_EQUIP_RESULT.empty() == false)
                        M_HP_LOG("heroespath::non-player::ownership::InventoryFactory::PopulateCreatureInventory[to equip - equip step](creature=\"" << creatureSPtr->ToString() << "\") unable to add the item \"" << NEXT_ITEM_SPTR->Name() << "\" \"" << NEXT_ITEM_SPTR->Desc() << "\"with reported error \"" << ITEM_EQUIP_RESULT << "\".  Proceeding...");
                }
            }
        }
        ...
        for (auto const & NEXT_ITEM_SPTR : ITEM_SVEC_PAIR.second)
        {
            const std::string ITEM_ADD_RESULT(creatureSPtr->ItemAdd(NEXT_ITEM_SPTR));
            if (ITEM_ADD_RESULT.empty() == false)
                M_HP_LOG("heroespath::non-player::ownership::InventoryFactory::PopulateCreatureInventory[not to equip](creature=\"" << creatureSPtr->ToString() << "\") unable to add the item \"" << NEXT_ITEM_SPTR->Name() << "\" \"" << NEXT_ITEM_SPTR->Desc() << "\" with reported error \"" << ITEM_ADD_RESULT << "\".  Proceeding...");
        }
    }
    ...
    const IItemSVecPair_t InventoryFactory::MakeItemSet_Clothing(const chance::ClothingChances & CHANCES)
    {
        ...
        if (COVER_TYPE != item::armor::cover_type::Count)
        {
            ...
            M_ASSERT_OR_LOGANDTHROW_SS((ITER != CHANCES.cover_map.end()), "heroespath::non_player::ownership::InventoryFactory::MakeItemSet_Clothing() failed to find \"" << COVER_TYPE << "\".");
                                                                ...
                                                                const non_player::CharacterSPtr_t & CREATURE_SPTR)
    {
        ...
        switch (randomSelectedWeaponType)
        {
            ...
            default:
            {
                ...
                ss << "heroespath::non_player::ownership::InventoryFactory::MakeItemSet_Weapons(creature_name=\"" << CREATURE_SPTR->Name() << "\") failed to find a valid random selected weapon.  (weapon_type=" << item::weapon_type::ToString(randomSelectedWeaponType, false) << "\")";
                                                              ...
                                                              const bool                          HAS_TWO_HANDED_WEAPON)
    {
        ...
        if (COVER_PAIR.second > 0)
        {
            ...
            M_ASSERT_OR_LOGANDTHROW_SS((COVER_CITER != CHANCES.cover_map.end()), "heroespath::non_player::ownership::InventoryFactory::MakeItemSet_Armor(creature_name=\"" << CREATURE_SPTR->Name() << "\") ARMOR_CHANCES.RandomCover() returned \"" << cover_type::ToString(COVER_PAIR.first) << "\", but that item was not found in the ARMOR_CHANCES.cover_map.");
        ...
        if (HELM_PAIR.second > 0)
        {
            ...
            M_ASSERT_OR_LOGANDTHROW_SS((HELM_CITER != CHANCES.helm_map.end()), "heroespath::non_player::ownership::InventoryFactory::MakeItemSet_Armor(creature_name=\"" << CREATURE_SPTR->Name() << "\") ARMOR_CHANCES.RandomHelm() returned \"" << helm_type::ToString(HELM_PAIR.first) << "\", but that item was not found in the ARMOR_CHANCES.helm_map.");
        ...
        if (SHIELD_PAIR.second > 0)
        {
            ...
            M_ASSERT_OR_LOGANDTHROW_SS((SHIELD_CITER != CHANCES.shield_map.end()), "heroespath::non_player::ownership::InventoryFactory::MakeItemSet_Armor(creature_name=\"" << CREATURE_SPTR->Name() << "\") ARMOR_CHANCES.RandomShield() returned \"" << shield_type::ToString(SHIELD_PAIR.first) << "\", but that item was not found in the ARMOR_CHANCES.shield_map.");
