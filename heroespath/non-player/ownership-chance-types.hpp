#ifndef APPBASE_HEROESPATH_NONPLAYER_OWNERSHIPCHANCETYPES_INCLUDED
#define APPBASE_HEROESPATH_NONPLAYER_OWNERSHIPCHANCETYPES_INCLUDED
//
...
namespace chance
{
    ...
    {
        M_ASSERT_OR_LOGANDTHROW_SS((MAP.empty() == false), "heroespath::non_player::ownership::chance::MappedRandomFloatChance(T=\"" << boost::typeindex::type_id<T>().pretty_name() << "\") called when the map was empty.");
        ...
        M_HP_LOG("WARNING:  heroespath::non_player::ownership::chance::MappedRandomFloatChance(T=\"" << boost::typeindex::type_id<T>().pretty_name() << "\") failed to random select.  Choosing first with a count of one by default.");
    ...
    std::pair<T, std::size_t> MappedRandomItemChance(const std::map<T, ItemChances> & MAP)
    {
        ...
        M_HP_LOG("WARNING:  heroespath::non_player::ownership::chance::MappedRandomItemChance(T=\"" << boost::typeindex::type_id<T>().pretty_name() << "\") failed random selection.  Choosing first with a count of one by default.");
...
#endif //APPBASE_HEROESPATH_NONPLAYER_OWNERSHIPCHANCETYPES_INCLUDED
