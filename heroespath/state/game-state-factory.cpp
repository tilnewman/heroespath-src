...
#include "heroespath/player/party-serialize-includes.hpp"
...
namespace state
{
    ...
    GameStateSSet_t GameStateFactory::LoadAllGames() const
    {
        ...
        for (auto const & NEXT_PATH_OBJ : pathVec)
        {
            ...
            {
                M_HP_LOG("heroespath::state::GameStateFactory::LoadAll() while trying to de-serialize/load saved game file \"" << NEXT_PATH_OBJ.string() << "\", threw std::exception(\"" << E.what() << "\")");
            }
            catch (...)
            {
                M_HP_LOG("heroespath::state::GameStateFactory::LoadAll() while trying to de-serialize/load saved game file \"" << NEXT_PATH_OBJ.string() << "\", threw UNKNOWN exception.");
            }
        }
    ...
    player::CharacterSSet_t GameStateFactory::LoadAllCharacters() const
    {
        ...
        for (std::size_t i(0); i < NUM_FILES; ++i)
        {
            ...
            {
                M_HP_LOG("heroespath::state::GameStateFactory::LoadAllCharacters(), while trying to de-serialize/load saved game file \"" << pathVec[i].string() << "\", threw std::exception(\"" << E.what() << "\")");
            }
            catch (...)
            {
                M_HP_LOG("heroespath::state::GameStateFactory::LoadAllCharacters(), while trying to de-serialize/load saved game file \"" << pathVec[i].string() << "\", threw UNKNOWN exception.");
            }
        }
        ...
        for (auto const & NEXT_PATH : pathVec)
        {
            ...
            try
            {
                ...
                if ((*nextCharacterSPtr) == (*CHAR_TO_DELETE_SPTR))
                {
                    ...
                    {
                        M_HP_LOG("heroespath::state::GameStateFactory::DeleteCharacter(" << CHAR_TO_DELETE_SPTR->Name() << "), while trying to delete " << NEXT_PATH.string() << ", failed with error code=" << errorCode);
                        return false;
                    }
                }
            }
            ...
            {
                M_HP_LOG("heroespath::state::GameStateFactory::DeleteCharacter(" << CHAR_TO_DELETE_SPTR->Name() << "), while trying to de-serialize/load saved game file \"" << NEXT_PATH.string() << "\", threw std::exception(\"" << E.what() << "\")");
            }
            catch (...)
            {
                M_HP_LOG("heroespath::state::GameStateFactory::DeleteCharacter(" << CHAR_TO_DELETE_SPTR->Name() << "), while trying to de-serialize/load saved game file \"" << NEXT_PATH.string() << "\", threw UNKNOWN exception.");
            }
        }
                                ...
                                const std::string &             EXT_STR) const
    {
        ...
        if (false == bfs::exists(DIR_OBJ))
        {
            ...
            M_ASSERT_OR_LOGANDTHROW_SS((bfs::create_directory(DIR_OBJ, ec)), "heroespath::GameStateFactory::Save() was unable to create the save game directory \"" << DIR_OBJ.string() << "\".  Error code=" << ec);
        }
        ...
        for (std::size_t i(0); ; ++i)
        {
            ...
            M_ASSERT_OR_LOGANDTHROW_SS((i < 10000), "heroespath::Save() failed to find an available save character filename after 10,000 tries.  Something is very wrong...");
        }
        ...
        {
            ss.str("");
            ss << "heroespath::state::GameStateFactory::Save() while trying to serialize/save game file \"" << nextFilePathObj.string() << "\", threw std::exception(\"" << E.what() << "\".";
            M_HP_LOG(ss.str());
        ...
        {
            ss.str("");
            ss << "heroespath::state::GameStateFactory::Save() while trying to serialize/save game file \"" << nextFilePathObj.string() << "\", threw an UNKNOWN exception.";
            M_HP_LOG(ss.str());
            throw std::runtime_error(ss.str());
        }
    }
