...
namespace weapon
{
    ...
    const WeaponDetails WeaponDetailLoader::LookupWeaponDetails(const std::string & NAME)
    {
        ...
        if (CITER == weaponDetailsMap_.end())
        {
            ...
            ss << "heroespath::item::weapon::WeaponDetailLoader::LookupWeaponDetails(\"" << NAME << "\")  failed to find that name in weaponDetailsMap_.";
    ...
    void WeaponDetailLoader::LoadDetailsForKey(const std::string & WEAPON_NAME)
    {
        ...
        M_ASSERT_OR_LOGANDTHROW_SS((fieldsVec.size() == 9), "heroespath::item::weapon::WeaponDetailsLoader::LoadDetailsForKey(weapon_name=\"" << WEAPON_NAME << "\") using key=\"" << KEY_STR << "\" found value=\"" << VALUE_STR << "\" but failed to find the required 9 comma separated fields.");
    ...
    int WeaponDetailLoader::StringFieldToInt(const std::string & FIELD_NAME, const std::string & NUM_STR)
    {
        ...
        M_ASSERT_OR_LOGANDTHROW_SS((result != ERROR_VAL), "heroespath::item::weapon::WeaponDetailsLoader::StringFieldToInt(field_name=\"" << FIELD_NAME << "\", num_str=\"" << NUM_STR << "\") failed to convert the " << FIELD_NAME << " field of \"" << NUM_STR << "\" into an int.");
