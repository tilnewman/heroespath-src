...
namespace player
{
    ...
    {
        M_ASSERT_OR_LOGANDTHROW_SS((CREATURE_CPTRC != nullptr), "heroespath::player::party::GetNextInOrder(WILL_CHOOSE_AFTER=" << std::boolalpha << WILL_CHOOSE_AFTER << ") was given a null CREATURE_CPTRC.");
        ...
        ss << "heroespath::player::party::GetNextInOrder(creature_name=" << CREATURE_CPTRC->Name() << ", WILL_CHOOSE_AFTER=" << std::boolalpha << WILL_CHOOSE_AFTER << ") but that CREATURE_CPTRC was not found in the party list.";
    ...
    {
        M_ASSERT_OR_LOGANDTHROW_SS((INDEX_NUM < charactersSVec_.size()), "heroespath::player::party::GetAtOrderPos(INDEX_NUM=" << INDEX_NUM << ") was given an out of range INDEX_NUM.  (it was too big, max is " << charactersSVec_.size() - 1 << ")");
        ...
        std::ostringstream ss;
        ss << "heroespath::player::party::GetAtOrderPos(INDEX_NUM=" << INDEX_NUM << ") never found an index matching what was given.";
        throw std::runtime_error(ss.str());
    }
    ...
    {
        M_ASSERT_OR_LOGANDTHROW_SS((CREATURE_CPTRC != nullptr), "heroespath::player::party::GetOrderNum() was given a null CREATURE_CPTRC.");
        ...
        std::ostringstream ss;
        ss << "heroespath::player::party::GetOrderNum(creature_name=" << CREATURE_CPTRC->Name() << ") never found that creature in the party vec.  Put another way, the function was given a creature that was not in the character vec.";
        throw std::runtime_error(ss.str());
    }
