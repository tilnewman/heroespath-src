...
namespace gui
{
    ...
    bool ConditionImageManager::Test()
    {
        ...
        if (condIndex < game::creature::condition::Count)
        {
            auto const ENUM{ static_cast<game::creature::condition::Enum>(condIndex) };
            auto const ENUM_STR{ game::creature::condition::ToString(ENUM) };
    ...
    sfml_util::TextureSPtr_t ConditionImageManager::Get(const game::creature::condition::Enum ENUM) const
    {
    ...
    const std::string ConditionImageManager::MakeFilename(const game::creature::condition::Enum ENUM) const
    {
        return boost::algorithm::to_lower_copy(game::creature::condition::ToString(ENUM)) + filenameExtension_;
    }


    const boost::filesystem::path ConditionImageManager::MakeFilepath(const game::creature::condition::Enum ENUM) const
    {
