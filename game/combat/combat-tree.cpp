...
namespace combat
{
    ...
    const CombatNodePVec_t CombatTree::FindNodesClosestOfType(const int ORIGIN_BLOCKING_POS, const bool WILL_FIND_PLAYERS) const
    {
        ...
        M_ASSERT_OR_LOGANDTHROW_SS((closestCombatNodesPVec.empty() == false), "game::combat::CombatTree::FindNodesClosestOfType(orig_blocking_pos=" << ORIGIN_BLOCKING_POS << ", will_find_players=" << std::boolalpha << WILL_FIND_PLAYERS << ")  unbale to find any CombatNodes closest.  This should never happen.");
